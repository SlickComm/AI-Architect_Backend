...
from app.lib.firestore import FirestoreManager
...
async def download_payment(request: Request):
    email = request.query_params.get("email")
    if not email:
        raise HTTPException(status_code=400, detail="email required")

    try:
        new_balance = fm.reduce_user_tokens(email, PRICE)
